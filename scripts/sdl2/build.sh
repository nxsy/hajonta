...
# binary
clang ${CPPFLAGS} ${WARNFLAGS} -F ~/Library/Frameworks/ -framework SDL2 -framework OpenGL -o build/debug/hajonta source/hajonta/platform/sdl2.cpp ${DEBUG_FLAGS} ${INCLUDES} -DHAJONTA_LIBRARY_NAME=libgame.dylib
clang ${CPPFLAGS} ${WARNFLAGS} -F ~/Library/Frameworks/ -framework SDL2 -framework OpenGL -o build/debug/editor source/hajonta/platform/sdl2.cpp ${DEBUG_FLAGS} ${INCLUDES} -DHAJONTA_LIBRARY_NAME=libeditor.dylib
clang ${CPPFLAGS} ${WARNFLAGS} -F ~/Library/Frameworks/ -framework SDL2 -framework OpenGL -o build/debug/ddsviewer source/hajonta/platform/sdl2.cpp ${DEBUG_FLAGS} ${INCLUDES} -DHAJONTA_LIBRARY_NAME=libddsviewer.dylib
